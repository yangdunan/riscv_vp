...
struct Options {
	...
	addr_t display_end_addr = display_start_addr + Display::addressRange;
	 ...
	 addr_t SOBEL_FILTER_R_ADDR = 0x80000000;
	 addr_t SOBEL_FILTER_RESULT_ADDR = 0x8FFFFFFF;
...
int sc_main(int argc, char **argv) {
	...
	SimpleBus<3, 13> bus("SimpleBus");// NR_OF_INITIATORS=3, NR_OF_TARGETS=12
	...
	SimpleSensor sensor("SimpleSensor", 2);//irq_number =2
	SimpleSensor2 sensor2("SimpleSensor2", 5);//irq_number=5
	BasicTimer timer("BasicTimer", 9);//irq_number=3
	SimpleMRAM mram("SimpleMRAM", opt.mram_image, opt.mram_size);
	SimpleDMA dma("SimpleDMA", 4);//irq_number=4
	Flashcontroller flashController("Flashcontroller", opt.flash_device);
	EthernetDevice ethernet("EthernetDevice", 7, mem.data, opt.network_device);//irq_number = 7
	Display display("Display");
	DebugMemoryInterface dbg_if("DebugMemoryInterface");


	/*add filter*/
	SobelFilter sobel_filter("SobelFilter",3);
	...
	bus.ports[3] = new PortMapping(opt.term_start_addr, opt.term_end_addr);//term 0x20000000~0x2000010
	bus.ports[4] = new PortMapping(opt.sensor_start_addr, opt.sensor_end_addr);//sensor 0x50000000~0x50001000
	bus.ports[5] = new PortMapping(opt.dma_start_addr, opt.dma_end_addr);//dma  0x70000000~0x70001000
	bus.ports[6] = new PortMapping(opt.sensor2_start_addr, opt.sensor2_end_addr);//sensor2 0x50002000~0x50004000
	bus.ports[7] = new PortMapping(opt.mram_start_addr, opt.mram_end_addr);//mram 0x60000000~0x6FFFFFFF
	bus.ports[8] = new PortMapping(opt.flash_start_addr, opt.flash_end_addr);//flash 0x71000000~71080000
	bus.ports[9] = new PortMapping(opt.ethernet_start_addr, opt.ethernet_end_addr);//ethernet 0x30000000~
	bus.ports[10] = new PortMapping(opt.display_start_addr, opt.display_end_addr);//0x72000000
	bus.ports[11] = new PortMapping(opt.sys_start_addr, opt.sys_end_addr);//0x02010000
	bus.ports[12] = new PortMapping(opt.SOBEL_FILTER_R_ADDR, opt.SOBEL_FILTER_RESULT_ADDR);
