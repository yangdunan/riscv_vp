...
struct EthernetDevice : public sc_core::sc_module {
	...
	void run() {
				/*hint*/
		//std::cout << "EthernetDevice_THREAD " << std::endl;
		while (!disabled) {
			//std::cout << "EthernetDevice_THREAD run_event.notify" << std::endl;
			run_event.notify(sc_core::sc_time(10000, sc_core::SC_US));
			//std::cout << "EthernetDevice_THREAD run_event.notify done" << std::endl;
			//std::cout << "EthernetDevice_THREAD sc_core::wait(run_event)" << std::endl;
			sc_core::wait(run_event);  // 10000 times per second by default
			//std::cout << "ethernet tigger time: " << sc_core::sc_time_stamp() << std::endl;

			//std::cout << "EthernetDevice_THREADsc_core::wait(run_event) done " << std::endl;
			// check if data is available on the socket, if yes store it in an
			// internal buffer
			if (!has_frame) {
				//std::cout << "EthernetDevice_THREAD !has_frame" << std::endl;
				...
				if (has_frame)
				{
					//std::cout << "EthernetDevice_THREAD !plic->gateway_trigger_interrupt(irq_number);" << std::endl;
