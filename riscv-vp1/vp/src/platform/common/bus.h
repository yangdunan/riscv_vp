...
struct SimpleBus : sc_core::sc_module {
	...
	void transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
		...
		//std::cout << "bus.h isock->b_transport adress =="<<std::hex<<addr <<std::dec <<std::endl;
...
struct PeripheralWriteConnector : sc_core::sc_module {
	...
	void transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
		...
		//std::cout << "DMA connector  bus.h isock->b_transport(trans, local_delay);" << std::endl;
		//std::cout<<"DMA connector  "<<std::hex<<trans.get_address()<<std::dec <<std::endl;
...
class BusLock : public bus_lock_if {
   ...
   public:
	...
	virtual void unlock(unsigned hart_id) override {
		//std::cout<<"bus.h unlock"<<std::endl;
