...
struct RegisterRange {
	...
	void process(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
		...
		if (cmd == tlm::TLM_READ_COMMAND) {
			//std::cout<<"[vp::plic] transport tlm::TLM_READ_COMMAND"<<std::endl;
			read(addr, ptr, len, trans, delay);
		} else if (cmd == tlm::TLM_WRITE_COMMAND) {
			//std::cout<<"[vp::plic] transport tlm::TLM_WRITE_COMMAND"<<std::endl;
			ensure(!readonly);
			write(addr, ptr, len, trans, delay);
		} else {
			//std::cout<<"[vp::plic] transport tlm::other"<<std::endl;
