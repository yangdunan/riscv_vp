...
struct CLINT : public clint_if, public sc_core::sc_module {
	...
	CLINT(sc_core::sc_module_name) {
		...
		/*hint*/
		//std::cout<<"SC_HAS_PROCESS(CLINT); "<<std::endl;
		SC_THREAD(run);
		
	}

	uint64_t update_and_get_mtime() override {
		//std::cout << "[vp::client] update_and_get_mtime"<< std::endl;
	...
	void run() {
		/*hint*/
		//std::cout << "CLINT_THREAD " << std::endl;
		while (true) {
			...
			for (unsigned i = 0; i < NumberOfCores; ++i) {
				auto cmp = mtimecmp[i];
				/*ignore*/ //std::cout << "[vp::clint] process mtimecmp[" << i << "]=" << cmp << ", mtime=" << mtime << std::endl;
				if (cmp > 0 && mtime >= cmp) {
					/*ignore*/// std::cout << "[vp::clint] set timer interrupt for core " << i << std::endl;
					target_harts[i]->trigger_timer_interrupt(true);
				} else {
					/*ignore*/ //std::cout << "[vp::clint] unset timer interrupt for core " << i << std::endl;
					...
					if (cmp > 0 && cmp < UINT64_MAX) {
						...
						/*ignore*/// std::cout << "[vp::clint] time=" << time << std::endl;
						/*ignore*/ //std::cout << "[vp::clint] goal=" << goal << std::endl;
						/*ignore*/ //std::cout << "[vp::clint] goal-time=delay=" << goal-time << std::endl;
	...
	bool pre_read_mtime(RegisterRange::ReadInfo t) {
		...
		//std::cout << "[vp::clint] pre_read_mtime"<< std::endl;
	...
	void post_write_mtimecmp(RegisterRange::WriteInfo t) {
		/*ignore*/// std::cout << "[vp::clint] write mtimecmp[addr=" << t.addr << "]=" << mtimecmp[t.addr / 8] << ", mtime=" <<
		/*ignore*/// mtime << std::endl;
		irq_event.notify(t.delay);
	}

	void post_write_msip(RegisterRange::WriteInfo t) {
		//std::cout << "[vp::clint] post_write_msip"<< std::endl;
		assert(t.addr % 4 == 0);
	...
	void transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
		delay += 2 * clock_cycle;
		//std::cout << "[vp::clint] transport"<< std::endl;
