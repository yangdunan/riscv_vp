...
struct SimpleSensor2 : public sc_core::sc_module {
	...
	SimpleSensor2(sc_core::sc_module_name, uint32_t irq_number) : irq_number(irq_number) {
		...
		SC_THREAD(run);
					/*hint*/
	std::cout<<"SC_HAS_PROCESS(SimpleSensor2); "<<std::endl;
		router
		    .add_register_bank({
	...
	void data_frame_access_callback(tlm::tlm_generic_payload &trans, sc_core::sc_time) {
		...
		std::cout<<" SimpleSensor2 data_frame_access_callback"<<std::endl;
		vp::map::execute_memory_access(trans, data_frame.data());
	}

	void register_access_callback(const vp::map::register_access_t &r) {
		// trigger pre read/write actions
		std::cout<<" SimpleSensor2 register_access_callback"<<std::endl;
	...
	void transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
				std::cout << "[vp::sensor2] transport"<< std::endl;
	...
	void run() {
				/*hint*/
		std::cout << "Simple_Sensor2_THREAD " << std::endl;
		while (true) {

			std::cout << "Simple_Sensor2_THREAD run_event.notify" << std::endl;
			run_event.notify(sc_core::sc_time(scaler, sc_core::SC_MS));
			std::cout << "Simple_Sensor2_THREAD run_event.notify done" << std::endl;
			std::cout << "Simple_Sensor2_THREAD wait(run_event)" << std::endl;
			sc_core::wait(run_event);  // 40 times per second by default
			std::cout << "simple sensor2 tigger time: " << sc_core::sc_time_stamp() << std::endl;

			std::cout << "Simple_Sensor2_THREAD wait(run_event) done" << std::endl;
			int i =0;
			// fill with random data
			for (auto &n : data_frame) {
				i++;
				//std::cout << "Simple_Sensor2_THREAD i == "<<i<< std::endl;
				if (filter == 1) {
					n = rand() % 10 + 48;
					std::cout << "Simple_Sensor2_THREAD filter == 1" << std::endl;
				} else if (filter == 2) {
					n = rand() % 26 + 65;
					std::cout << "Simple_Sensor2_THREAD filter == 2" << std::endl;
				} else {
					// fallback for all other filter values
					n = rand();  // random char
					//std::cout << "Simple_Sensor2_THREAD filter == 3" << std::endl;
				}
			}
